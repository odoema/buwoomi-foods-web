...
async function updateProfile(fields) {
  if (window.BuwoomiApi && window.BuwoomiApi.enabled()) {
    return window.BuwoomiApi.updateMe(fields);
  }
  if (!sb) throw new Error("Backend not configured");
...
async function fetchAllMenuItems() {
  if (window.BuwoomiApi && window.BuwoomiApi.enabled()) {
    return window.BuwoomiApi.adminMenu();
  }
  if (!sb) return null;
...
async function saveMenuItem(item) {
  if (window.BuwoomiApi && window.BuwoomiApi.enabled()) {
    return window.BuwoomiApi.saveMenuItem(item);
  }
  if (!sb) throw new Error("Backend not configured");
  const payload = {
...
async function deleteMenuItem(id) {
  if (window.BuwoomiApi && window.BuwoomiApi.enabled()) {
    return window.BuwoomiApi.deleteMenuItem(id);
  }
  if (!sb) throw new Error("Backend not configured");
...
async function saveExtra(extra) {
  if (window.BuwoomiApi && window.BuwoomiApi.enabled()) {
    return window.BuwoomiApi.saveExtra(extra);
  }
  if (!sb) throw new Error("Backend not configured");
...
async function deleteExtra(id) {
  if (window.BuwoomiApi && window.BuwoomiApi.enabled()) {
    return window.BuwoomiApi.deleteExtra(id);
  }
  if (!sb) throw new Error("Backend not configured");
...
async function fetchCategories() {
  if (window.BuwoomiApi && window.BuwoomiApi.enabled()) {
    return window.BuwoomiApi.categories();
  }
  if (!sb) return null;
...
async function saveCategory(cat) {
  if (window.BuwoomiApi && window.BuwoomiApi.enabled()) {
    return window.BuwoomiApi.saveCategory(cat);
  }
  if (!sb) throw new Error("Backend not configured");
...
async function deleteCategory(id) {
  if (window.BuwoomiApi && window.BuwoomiApi.enabled()) {
    return window.BuwoomiApi.deleteCategory(id);
  }
...
async function fetchExtras() {
  if (window.BuwoomiApi && window.BuwoomiApi.enabled()) {
    return window.BuwoomiApi.extras();
  }
  if (!sb) return null;
...
async function saveAddress({ id, label, line1, city, isDefault, latitude, longitude }) {
  if (window.BuwoomiApi && window.BuwoomiApi.enabled()) {
    return window.BuwoomiApi.saveAddress({ id, label, line1, city, isDefault, latitude, longitude });
  }
  if (!sb) throw new Error("Backend not configured");
  const session = await getSession();
  ...
  if (latitude != null && Number.isFinite(Number(latitude))) payload.latitude = Number(latitude);
  if (longitude != null && Number.isFinite(Number(longitude))) payload.longitude = Number(longitude);
  const query = id
...
async function deleteAddress(id) {
  if (window.BuwoomiApi && window.BuwoomiApi.enabled()) {
    return window.BuwoomiApi.deleteAddress(id);
  }
  if (!sb) throw new Error("Backend not configured");
...
async function fetchFavorites() {
  if (window.BuwoomiApi && window.BuwoomiApi.enabled()) {
    return window.BuwoomiApi.favorites();
  }
  if (!sb) return null;
  const session = await getSession();
  if (!session) return [];
...
async function toggleFavorite(menuItemId, enabled) {
  if (window.BuwoomiApi && window.BuwoomiApi.enabled()) {
    return window.BuwoomiApi.toggleFavorite(menuItemId, enabled);
  }
  if (!sb) throw new Error("Backend not configured");
  const session = await getSession();
  if (!session) throw new Error("Please sign in first.");
...
async function fetchSavedPaymentMethods() {
  if (window.BuwoomiApi && window.BuwoomiApi.enabled()) {
    return window.BuwoomiApi.paymentMethods();
  }
  if (!sb) return null;
  const session = await getSession();
  if (!session) return [];
...
async function savePaymentMethod({ id, methodType, label, last4, isDefault }) {
  if (window.BuwoomiApi && window.BuwoomiApi.enabled()) {
    return window.BuwoomiApi.savePaymentMethod({ id, methodType, label, last4, isDefault });
  }
  if (!sb) throw new Error("Backend not configured");
  const session = await getSession();
  if (!session) throw new Error("Please sign in first.");
...
async function deletePaymentMethod(id) {
  if (window.BuwoomiApi && window.BuwoomiApi.enabled()) {
    return window.BuwoomiApi.deletePaymentMethod(id);
  }
  if (!sb) throw new Error("Backend not configured");
...
async function fetchNotifications() {
  if (window.BuwoomiApi && window.BuwoomiApi.enabled()) {
    return window.BuwoomiApi.notifications();
  }
  if (!sb) return null;
  const session = await getSession();
  if (!session) return [];
...
async function markNotificationRead(id) {
  if (window.BuwoomiApi && window.BuwoomiApi.enabled()) {
    return window.BuwoomiApi.markNotificationRead(id);
  }
  if (!sb) throw new Error("Backend not configured");
...
async function createNotification({ title, body, kind = "general" }) {
  if (window.BuwoomiApi && window.BuwoomiApi.enabled()) {
    return window.BuwoomiApi.createNotification({ title, body, kind });
  }
  if (!sb) return null;
  const session = await getSession();
  if (!session) return null;
...
async function placeOrder({ cart, paymentMethod, addressId }) {
  if (window.BuwoomiApi && window.BuwoomiApi.enabled()) {
    return window.BuwoomiApi.placeOrder({ cart, paymentMethod, addressId });
  }
  ...
  if (error) throw error;
  return { orderNo: data.orderNo, orderId: data.orderId, subtotal: data.subtotal, fee: data.fee, total: data.total, persisted: true, via: "supabase-rpc" };
}

async function fetchOrders() {
  if (window.BuwoomiApi && window.BuwoomiApi.enabled()) {
    return window.BuwoomiApi.orders();
  }
  if (!sb) return null;
  const session = await getSession();
  if (!session) return [];
...
async function uploadMenuImage(file, itemId) {
  if (window.BuwoomiApi && window.BuwoomiApi.enabled()) {
    return window.BuwoomiApi.uploadMenuImage(file, itemId);
  }
  if (!sb) throw new Error("Backend not configured");
...
async function fetchSettings() {
  if (window.BuwoomiApi && window.BuwoomiApi.enabled()) {
    return window.BuwoomiApi.settings();
  }
  if (!sb) return null; const {data,error}=await sb.from("app_settings").select("*").eq("id","default").maybeSingle(); if(error) throw error; return data;
}
async function saveSettings(settings) {
  if (window.BuwoomiApi && window.BuwoomiApi.enabled()) {
    return window.BuwoomiApi.saveSettings(settings);
  }
  if(!sb) throw new Error("Backend not configured"); const session=await getSession(); if(!session) throw new Error("Please sign in first."); const payload={...settings, id:"default", updated_by:session.user.id, updated_at:new Date().toISOString()}; const {data,error}=await sb.from("app_settings").upsert(payload,{onConflict:"id"}).select().single(); if(error) throw error; return data;
}
async function fetchAllOrders() {
  if (window.BuwoomiApi && window.BuwoomiApi.enabled()) {
    return window.BuwoomiApi.adminOrders();
  }
  if(!sb) return []; const {data,error}=await sb.from("orders").select("*, order_items(*), profiles(full_name,phone)").order("placed_at",{ascending:false}); if(error) throw error; return data||[];
}
async function updateOrderStatus(id,status) {
  if (window.BuwoomiApi && window.BuwoomiApi.enabled()) {
    return window.BuwoomiApi.updateOrderStatus(id, status);
  }
  if(!sb) throw new Error("Backend not configured"); const {data:order,error}=await sb.from("orders").update({status}).eq("id",id).select("id,user_id,order_no,status").single(); if(error) throw error; const titles={placed:"Order received",preparing:"Your meal is being prepared",out_for_delivery:"Your order is on the way",delivered:"Order delivered",cancelled:"Order cancelled"}; const bodies={placed:`${order.order_no} has been received.`,preparing:`${order.order_no} is being prepared.`,out_for_delivery:`${order.order_no} is out for delivery.`,delivered:`${order.order_no} has been delivered.`,cancelled:`${order.order_no} has been cancelled.`}; await sb.from("notifications").insert({user_id:order.user_id,title:titles[status]||"Order update",body:bodies[status]||`Your order ${order.order_no} was updated.`,kind:"order"}); return order; }
async function saveOrderRating(id,rating,comment) {
  if (window.BuwoomiApi && window.BuwoomiApi.enabled()) {
    return window.BuwoomiApi.rate(id, rating, comment);
  }
  if(!sb) return; const {error}=await sb.from("orders").update({rating,rating_comment:comment||null}).eq("id",id); if(error) throw error;
}
