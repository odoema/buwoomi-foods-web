...
function views() {
  ...
  const preferredIds = {
    Popular: ["gcb", "ug-rolex", "ug-fried-chicken"], Chicken: ["ug-fried-chicken", "gcb", "cw"],
    "Burgers & Wraps": ["cb", "cheeseburger", "chickenwrap"], Beef: ["bs", "beefwrap", "ug-beef-pizza"],
    Veggie: ["vp", "veggieburger"], Sides: ["fries", "ug-chips"], Snacks: ["ug-muchomo", "samosa", "springrolls"],
    Pizza: ["chickenpizza", "ug-pizza", "beefpizza"], Drinks: ["fj", "soda", "water"], Desserts: ["icecream", "cake"],
    Rolex: ["ug-rolex", "ug-chicken-rolex", "ug-beef-rolex"], "Chips & Meals": ["ug-chips", "ug-chips-chicken", "ug-chips-egg"],
    Breakfast: ["ug-mandazi", "ug-tea", "ug-egg-roll", "ug-chapati"], "Local Favourites": ["ug-matooke-beef", "ug-pilau-chicken", "ug-katogo"]
  };
  const norm = (v) => String(v || "").trim().toLowerCase();
  const used = new Set();
  const allWithImages = MENU.filter(m => m?.img);
  const artByCategory = {};
  CATEGORIES.forEach(c => {
    const candidates = [];
    for (const id of (preferredIds[c] || [])) { const item = MENU.find(m => m.id === id && m.img); if (item && !candidates.includes(item)) candidates.push(item); }
    MENU.filter(m => norm(m.cat) === norm(c) && m.img).forEach(item => { if (!candidates.includes(item)) candidates.push(item); });
    let chosen = candidates.find(item => !used.has(item.img));
    if (!chosen) chosen = allWithImages.find(item => !used.has(item.img));
    if (chosen) { artByCategory[c] = chosen.img; used.add(chosen.img); }
  });
  return CATEGORIES.map(c => {
    const artUrl = String(artByCategory[c] || "").replace(/\x27/g, "%27");
    const fallbackIcon = CATEGORY_ICON[c] || "leaf";
    const photoStyle = artUrl ? "style=\"background-image:url(\x27" + artUrl + "\x27) !important;background-position:center !important;background-size:cover !important;background-repeat:no-repeat !important\"" : "style=\"background-image:none !important\"";
    return "<button class=\"cat-icon-btn " + (state.cat === c ? "on" : "") + "\" data-cat=\"" + esc(c) + "\" aria-label=\"Browse " + esc(c) + "\"><span class=\"circle-ic food-category-photo " + (artUrl ? "" : "food-category-icon-fallback") + "\" " + photoStyle + ">" + (artUrl ? "" : icon(fallbackIcon)) + "</span><span>" + esc(c) + "</span></button>";
  }).join("");
})()}</div></div>
        `} `<button class="home-cart-bar" data-go="cart"><span><strong>${cartCount()} ${cartCount() === 1 ? "item" : "items"}</strong><small>Ready in your cart</small></span><b>${ugx(cartTotals().total)}</b><span class="home-cart-action">View cart ${icon("chevronRight")}</span></button>` : ""}
        ${tabbar("home")}
      </div>`;
    },
