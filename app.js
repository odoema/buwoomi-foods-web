...
function views() {
  ...
  return `<button class="cat-icon-btn ${state.cat === c ? "on" : ""}" data-cat="${c}" aria-label="Browse ${esc(c)}"><span class="circle-ic food-category-photo" style="background-image:url('assets/category-icons.webp');background-position:${spritePos};background-size:500% 200%;background-repeat:no-repeat"></span><span>${c}</span></button>`;
}).join("")}</div></div>
