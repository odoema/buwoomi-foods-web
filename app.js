...
function bindNav() {
  document.querySelectorAll("[data-go]").forEach((b) => {
    b.onclick = () => {
      ...
      const dir = tabIds.includes(id) ? "fade" : (b.classList.contains("icon-btn") || b.dataset.back ? "back" : "forward");
  ...
  if (!backendOn && ["addresses", "payments", "favorites", "notifications", "orders"].includes(section)) {
    ...
    body = `
      <div class="section">
        ...
        <div class="summary">
          <h4>Add Address</h4>
          <div class="field"><label>Label</label><input id="addressLabel" placeholder="Home" /></div>
          <div class="field"><label>Address</label><input id="addressLine1" placeholder="Acacia Avenue, Kampala" /></div>
          <div class="field"><label>City</label><input id="addressCity" placeholder="Kampala" value="Kampala" /></div>
          <label style="display:flex;gap:8px;align-items:center;font-size:13px;margin:10px 0"><input id="addressDefault" type="checkbox" /> Make default</label>
...
function views() {
  ...
  const v = {
    splash: () => `
      <div class="splash">
        <div class="splash-brand"><img src="assets/logo-on-green.png" alt="BUWOOMI FOODS LTD" width="280" height="180" /></div>
        <div class="tag">Good Food. Closer to You.</div>
        <button class="splash-continue" data-go="onb1">Continue</button>
      </div>`,

    onb1: () => `
      <div class="onb">
        <div class="onb-top"><button class="back-link" data-go="splash" data-back="1">${icon("back")}<span>Back</span></button><button class="skip" data-go="login">Skip</button></div>
    ...
    onb2: () => `
      <div class="onb">
        <div class="onb-top"><button class="back-link" data-go="onb1" data-back="1">${icon("back")}<span>Back</span></button><button class="skip" data-go="login">Skip</button></div>
    ...
    onb3: () => `
      <div class="onb">
        <div class="onb-top"><button class="back-link" data-go="onb2" data-back="1">${icon("back")}<span>Back</span></button><button class="skip" data-go="login">Skip</button></div>
...
function addToCart(id, opts = {}) {
  ...
  if (existing && !opts.custom) {
    ...
    state.cart.push({ ...p, baseId: id, qty: opts.qty || 1, price: opts.price ?? p.price, name: opts.name || p.name });
  }
  persistCart();
...
function bind() {
  ...
  if (addCart) addCart.onclick = () => {
    ...
    addToCart(p.id, { custom: true, qty: state.qty, price: p.price + sizeExtra + extrasTotal, name });
    go("cart", {}, "forward");
  };
  ...
  if (saveAddressBtn) saveAddressBtn.onclick = async () => {
    ...
    try {
      await window.BuwoomiBackend.saveAddress({
        label: $("#addressLabel")?.value.trim() || "Home",
        line1,
        city: $("#addressCity")?.value.trim() || "Kampala",
  ...
  if (authSubmit) authSubmit.onclick = async () => {
    ...
    try {
      ...
      if (state.authMode === "signup" && !result.session) { state.authError = "Account created. Check your email to confirm your account, then sign in."; render(); return; }
      go("home", {}, "forward");
    } catch (e) {
      state.authBusy = false;
  ...
  if (place) place.onclick = async () => {
    place.disabled = true;
    const { sub, fee, total } = cartTotals();
    if (sub < Number(state.settings?.minimum_order_ugx || 0)) { place.disabled=false; return showToast(`Minimum order is ${ugx(Number(state.settings.minimum_order_ugx))}.`); }
    if (!state.addressId && state.session) { place.disabled=false; return showToast("Please select a delivery address first."); }
    try {
      const backendOn = !!(window.BuwoomiBackend && window.BuwoomiBackend.ready);
      const result = backendOn
        ? await window.BuwoomiBackend.placeOrder({
            cart: state.cart, paymentMethod: state.pay, addressId: state.addressId,
          })
        : { orderNo: `BW-${new Date().getFullYear()}-` + String(Math.floor(Math.random() * 999999)).padStart(6, "0") };
      state.orderNo = result.orderNo; state.orderId = result.orderId || null;
      state.cart = [];
      persistCart();
      if (backendOn && result.persisted) {
        try { await window.BuwoomiBackend.createNotification({ title: "Order placed", body: `${result.orderNo} has been received and is being prepared.`, kind: "order" }); } catch (_) {}
      }
...
render();
syncMenuFromBackend();
(async()=>{ try { const s=await window.BuwoomiBackend?.fetchSettings?.(); if(s){state.settings=s;state.adminSettings=s;render();} } catch(e){} })();
setTimeout(() => { if (state.screen === "splash") go("onb1", {}, "fade"); }, 1800);
